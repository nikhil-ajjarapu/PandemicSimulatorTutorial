...
from ..environment import LocationID, PersonRoutine, HairSalon, Restaurant, Bar, PlaceOfWorship, \
...
class DefaultPersonRoutineAssignment(PersonRoutineAssignment):
    ...
    @staticmethod
    def get_minor_routines(home_id: LocationID, age: int, is_religious: bool) -> Sequence[PersonRoutine]:
        routines = [
    ...
    @staticmethod
    def get_retired_routines(home_id: LocationID, is_religious: bool) -> Sequence[PersonRoutine]:
        routines = [
    ...
    @staticmethod
    def get_worker_outside_work_routines(home_id: LocationID, is_religious: bool) -> Sequence[PersonRoutine]:
        routines = [
            triggered_routine(None, GroceryStore, 7),
            triggered_routine(None, RetailStore, 7),
            triggered_routine(None, HairSalon, 30),
    ...
    def assign_routines(self, persons: Sequence[Person]) -> None:
        for p in persons:
            if isinstance(p, Retired):
                p.set_routines(self.get_retired_routines(p.home, p.is_religious))
            elif isinstance(p, Minor):
                p.set_outside_school_routines(self.get_minor_routines(p.home, p.id.age, p.is_religious))
            elif isinstance(p, Worker):
                p.set_during_work_routines(self.get_worker_during_work_routines(p.work))
                p.set_outside_work_routines(self.get_worker_outside_work_routines(p.home, p.is_religious))
